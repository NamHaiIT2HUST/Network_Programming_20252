...
// Hàm xử lý riêng cho từng Luồng (Thread)
void *client_handler(void *arg) {
    // Ép kiểu và lấy giá trị File Descriptor, sau đó giải phóng bộ nhớ ngay
    int client = *(int *)arg;
    free(arg); 
    ...
    // Quét file (Đã fix lỗi tương thích trên WSL)
    ...
    printf("Client %d disconnected from thread.\n", client);
    close(client);
    pthread_exit(NULL); // Dọn dẹp và thoát luồng
}
...
int main() {
    ...
    printf("File Server (Multithreading) is listening on port 9000...\n");
    ...
    while (1) {
        ...
        // Cấp phát động để tránh Race Condition cho luồng
        int *new_sock = malloc(sizeof(int));
        *new_sock = client;

        pthread_t tid;
        // Tạo luồng mới và truyền con trỏ new_sock vào
        pthread_create(&tid, NULL, client_handler, (void *)new_sock);
        
        // Tự động thu hồi tài nguyên khi luồng chạy xong (chống rò rỉ RAM)
        pthread_detach(tid); 
    }
